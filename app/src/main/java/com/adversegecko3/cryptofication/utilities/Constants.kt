package com.adversegecko3.cryptofication.utilities

object Constants {
    const val PREF_CURRENCY = "prefCurrency"
    const val PREF_SCHEME = "prefScheme"
    const val PREF_FILTER_OPTION = "prefDefaultFilterOption"
    const val PREF_FILTER_ORDER = "prefDefaultFilterOrder"
    const val PREF_ITEMS_PAGE = "prefDefaultItemsPage"
    const val PREF_SCROLL_ON_PAGE_CHANGED = "prefScrollOnPageChanged"
    const val PREF_ALERT_TIME = "prefAlertTime"
    const val PREF_INFO_NOTIFICATIONS = "prefInfoNotifications"
    const val PREF_ABOUT = "prefAbout"
    const val PREF_CREDITS = "prefCredits"
    const val PREF_VERSION = "prefVersion"
    const val PREF_FIRST_RUN = "prefFirstRun"
    const val PREF_DB_HAS_ITEMS = "prefDBHasItems"
    const val CHANNEL_ID = "CryptOficatioNChannel"
    const val ITEMS_PER_AD = 20
}